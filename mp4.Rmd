...
mp4<-finallist%>%
  inner_join(remakesoriginals, by="title")%>%
  select(-type)
mp4$votes <-as.integer(mp4$votes)
mp4$rating <-as.integer(mp4$rating)
```
...
```{r, message=FALSE, warning=FALSE}
# visualization
plot1<- ggplot(data = full, aes(x=title, y=rating.y))+
...
```{r}
ggplot(data = full, aes(x=diff, y = diff_votes)) + 
  geom_point() +
  geom_jitter()
```

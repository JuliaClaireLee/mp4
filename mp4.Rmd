...
sequels<-db %>%
  dbGetQuery("SELECT t.id, t.title, t.production_year, 
  mi1.info AS votes, mi2.info AS rating, n.name AS director, n2.name AS writer 
FROM title t
Join cast_info AS ci on ci.movie_id = t.id
Join cast_info AS ci2 on ci2.movie_id = t.id
JOIN name AS n2  on n2.id = ci2.person_id
...
OR t.title = 'The Hangover Part III'
OR t.title = 'Shrek the Third'
OR t.title ='Pirates of the Caribbean: On Stranger Tides'
group by t.title
ORDER BY mi2.info;")
View(sequels)
```
